...
# Передача оргтехники в отдел с обработкой ошибки ввода данных
try:
    quantity = int(input("Enter quantity to transfer to department: "))
    if quantity <= 0:
        raise InvalidInputError("Invalid quantity")
    warehouse.transfer_item(printer, quantity, department)
except ValueError:
